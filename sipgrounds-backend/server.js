...
module.exports.authOrToken = authOrToken;
...
app.get('/api/users/profile', authOrToken, catchAsync(async (req, res) => {
    ...
    // Aggregate stats
    const [cafeCount, reviewCount, totalPointsEarned] = await Promise.all([
        Cafe.countDocuments({ author: user._id }),
        Review.countDocuments({ author: user._id }),
        Order.aggregate([
            { $match: { user: user._id, 'payment.paid': true } },
            { $group: { _id: null, totalPoints: { $sum: '$totalPointsEarned' } } }
        ])
    ]);
        ...
        data: {
            user: {
                ...
                stats: {
                    cafes: cafeCount,
                    reviews: reviewCount,
                    totalPointsEarned: totalPointsEarned[0]?.totalPoints || 0,
                    favoritesCafes: user.favoritecafes?.length || 0
                }
            }
        }
    });
...
app.get('/api/cafes', catchAsync(async (req, res) => {
    ...
    if (minPrice || maxPrice) {
        // Since we use priceRange ($, $$, $$$, $$$$), we map numeric prices to ranges
        const priceRanges = [];
        if (minPrice <= 10) priceRanges.push('$');
        if (minPrice <= 20) priceRanges.push('$$');
        if (minPrice <= 30) priceRanges.push('$$$');
        if (!maxPrice || maxPrice >= 40) priceRanges.push('$$$$');
    ...
    res.json({
        ...
        data: {
            cafes,
            campgrounds: cafes, // Backward compatibility
            pagination: {
...
// Get single cafe/campground by ID (shared handler for both routes)
const getCafeById = catchAsync(async (req, res) => {
    const campground = await Cafe.findById(req.params.id)
    ...
    res.json({
        success: true,
        data: {
            cafe: campground,        // For new frontend
            campground,              // For backward compatibility
            stats: {
...
app.post('/api/campgrounds', isLoggedIn, catchAsync(async (req, res) => {
    ...
    try {
        ...
        const campground = new Cafe(campgroundData);
...
app.put('/api/campgrounds/:id', isLoggedIn, isAuthor, catchAsync(async (req, res) => {
    const campground = await Cafe.findByIdAndUpdate(req.params.id, req.body, {
        new: true,
        runValidators: true
    }).populate('author', 'username');
...
// Reviews Routes (shared handler for both cafe and campground routes)
const createReview = catchAsync(async (req, res) => {
    ...
    // Award 2 points for writing a review
    const user = await User.findById(req.user._id);
    if (user) {
        await user.addPoints(2, 'review_written', campground._id);
    }

    res.status(201).json({
        success: true,
        data: { review },
        message: 'Review added successfully! You earned 2 points.'
    });
});
...
// Both routes for compatibility
app.post('/api/cafes/:id/reviews', isLoggedIn, createReview);
app.post('/api/campgrounds/:id/reviews', isLoggedIn, createReview);

app.delete('/api/cafes/:id/reviews/:reviewId', isLoggedIn, isReviewAuthor, deleteReview);
app.delete('/api/campgrounds/:id/reviews/:reviewId', isLoggedIn, isReviewAuthor, deleteReview);
...
app.post('/api/campgrounds/:id/bookings', authOrToken, catchAsync(async (req, res) => {
    ...
    const campground = await Cafe.findById(id);
    if (!campground) {
    ...
    console.log('Cafe found:', { title: campground.title, price: campground.price });
...
app.get('/api/campgrounds/search/:term', catchAsync(async (req, res) => {
    ...
    const campgrounds = await Cafe.find({
        $or: [
...
app.get('/api/campgrounds/category/:type', catchAsync(async (req, res) => {
    ...
    const campgrounds = await Cafe.find({ title: regex })
        .populate('author', 'username')
        .limit(parseInt(limit))
        .select('title location price images description');
...
app.get('/api/stats', catchAsync(async (req, res) => {
    ...
    const priceStats = await Cafe.aggregate([
        {
            $group: {
                _id: null,
                avgPrice: { $avg: '$price' },
...
app.post('/api/orders', authOrToken, catchAsync(async (req, res) => {
    const { items, shippingAddress, userId, usePoints } = req.body;
    ...
    // If paying with points, override simulated payment and redeem
    if (usePoints) {
        ...
        await order.save();
        await user.redeemPoints(Math.floor(totalAmount), `Order ${order.orderNumber || order._id} - Paid with points`, order._id);
    }
    
    // Award points for shop orders (1 point per amount spent)
    try {
        const pointsToAward = Math.floor(order.totalAmount || 0);
        if (pointsToAward > 0) {
            // Persist on order for consistency with other flows
            if (order.totalPointsEarned !== pointsToAward) {
                order.totalPointsEarned = pointsToAward;
                await order.save();
            }
            const awardingUserId = order.user?._id || orderUserId;
            const awardingUser = await User.findById(awardingUserId);
            if (awardingUser) {
                await awardingUser.addPoints(
                    pointsToAward,
                    `Order ${order.orderNumber || order._id} - Shop purchase`,
                    order._id
                );
            }
        }
    } catch (e) {
        console.error('Failed to award points for shop order:', e);
    }
...
app.get('/api/orders', isLoggedIn, catchAsync(async (req, res) => {
    const orders = await Order.find({ user: req.user._id })
        ...
        .populate('cafe', 'name location address contact')
        .sort({ createdAt: -1 });
...
app.get('/sitemap.xml', catchAsync(async (req, res) => {
    ...
    const campgrounds = await Cafe.find({}, '_id title updatedAt').sort({ updatedAt: -1 });
...
app.get('/robots.txt', (req, res) => {
    ...
    const robotsTxt = `# Robots.txt for The Cafes
User-agent: *
Allow: /
...
app.use('/api/menu-items', menuRoutes); // Alternative endpoint for menu items
...
app.listen(port, () => {
    console.log(`🚀 The Cafes Backend API running on port ${port}`);
