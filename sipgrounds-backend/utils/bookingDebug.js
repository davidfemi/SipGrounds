...
class BookingDebug {
    ...
    static async validateCafe(campgroundId) {
        try {
            const campground = await Cafe.findById(campgroundId);
            
            if (!campground) {
                return { valid: false, error: 'Cafe not found', code: 'NOT_FOUND' };
            }
            ...
            if (campground.peopleBooked >= campground.capacity) {
                issues.push('Cafe is fully booked');
            }
    ...
    static async getCafeBookingStats(campgroundId) {
        try {
            const campground = await Cafe.findById(campgroundId);
            if (!campground) {
